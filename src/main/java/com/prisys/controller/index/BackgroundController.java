package com.prisys.controller.index;
...
import com.prisys.util.MD5Util;
...
public class BackgroundController extends BaseController {
	...
	public String index(Model model) throws Exception {
		...
		UserFormMap userFormMapJudge = (UserFormMap) Common.findUserSession(request);
		
		if(userFormMapJudge==null){	//避免重复设置
			//权限信息设置 start
			String username = (String) SecurityUtils.getSubject().getPrincipal();


			UserFormMap userFormMapInit = new UserFormMap();
			userFormMapInit.put("username", "" + username + "");
			List<UserFormMap> userFormMaps =new ArrayList<UserFormMap>();
			try{
				 userFormMaps = userMapper.seletUser(userFormMapInit);
			}catch(Exception e){
				e.printStackTrace();
			}
...
//						.getAttribute("userSessionId").toString();
				Integer userId = null ;
				try{
					 userId = (Integer) userFormMaps.get(0).get("id");
				}catch(Exception e){
					
				}
				
				List<ResFormMap> rs = resourcesMapper.findUserResourcess(userId.toString());
		...
		// (Object) request.getSession().getAttribute("userSession");
	 ...
	 * 获取某个用户的权限资源
	 * 
	 * @author zqb Email： date：2014-3-4
	 * @param request
	 * @return
	 */
